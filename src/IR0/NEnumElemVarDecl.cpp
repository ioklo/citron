...
std::optional<RMember> NEnumElemVarDecl::ResolveIdentifier(const RName& name, size_t explicitTypeParamsExceptOuterCount, IR0Factory& factory)
{
    // VarDecl의 자식이 ResolveIdentifier를 호출할 수 없고, bodyspace도 아니기 때문에 직접 호출할 일이 없다
    throw RuntimeFatalException();
}

RType* NEnumElemVarDecl::GetDeclType(RTypeArguments& typeArgs, IR0Factory& factory)
{
