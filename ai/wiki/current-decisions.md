...
- generic application과 trait signature matching을 위한 별도 `SmType`, `SmAppliedDecl`, application용 `SmTypeEnv` 계층은 제거한다. SmTranslator도 `RType`, `RAppliedDecl`, `RTypeArguments`를 직접 사용한다. formal type environment는 의미와 합성 법칙을 설명하는 데 사용하되 구현에서는 flattened complete argument 배열로 erase한다.
- declaration 자신에게 남은 type parameter가 없는 lambda, variable, enum element는 lexical outer arguments까지 적용된 뒤 `RAppliedDecl`로 나타낸다. 반대로 member 자신의 type arguments를 아직 받아야 하는 nominal type과 callable lookup은 `ROuterAppliedDecl` 또는 partial-application 상태를 유지한다. 다만 trait requirement signature 비교에서는 새 type variable을 해당 function member argument로 적용해 `RAppliedDecl`을 만든다.
