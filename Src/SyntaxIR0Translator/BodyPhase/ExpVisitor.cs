...
struct ExpVisitor
{
    ...
    public ExpVisitor(ScopeContext context)
    {
        this.context = context;
    }    

    // x
    ExpResult AnalyzeIdExp(S.IdentifierExp idExp, IType? hintType)
    {
        var typeArgs = BodyMisc.MakeTypeArgs(idExp.TypeArgs, context);
        var result = IdExpIdentifierResolver.Resolve(idExp.Value, typeArgs, hintType, context);
    ...
    {
        return new ExpResult.Exp(new R.BoolLiteralExp(boolExp.Value, context.GetBoolType()));
    }

    ExpResult.Exp AnalyzeIntLiteralExp(S.IntLiteralExp intExp)
    {
        return new ExpResult.Exp(new R.IntLiteralExp(intExp.Value, context.GetIntType()));
    }
    ...
    ExpResult.Exp AnalyzeStringExp(S.StringExp stringExp)
    {
        ...
        return new ExpResult.Exp(new R.StringExp(builder.ToImmutable(), context.GetStringType()));
    }
    ...
    ExpResult.Exp AnalyzeIntUnaryAssignExp(S.Exp operand, R.InternalUnaryAssignOperator op)
    {
        ...
        if (operandResult is ExpResult.Loc locResult)
        {
            ...
            if (!context.IsIntType(locResult.TypeSymbol))
    ...
    ExpResult.Exp AnalyzeUnaryOpExp(S.UnaryOpExp unaryOpExp)
    {
        ...
        switch (unaryOpExp.Kind)
        {
            case S.UnaryOpKind.LogicalNot:
                {
                    // exact match
                    if (!context.IsBoolType(operandExp.GetTypeSymbol()))
            ...
            case S.UnaryOpKind.Minus:
                {
                    if (!context.IsIntType(operandExp.GetTypeSymbol()))
    ...
    ExpResult.Exp AnalyzeAssignBinaryOpExp(S.BinaryOpExp exp)
    {
        ...
        if (destResult is ExpResult.Loc destLocResult)
        {
            ...
            IType destType = destLocResult.TypeSymbol;
    ...
    ExpResult.Exp AnalyzeBinaryOpExp(S.BinaryOpExp binaryOpExp)
    {
        ...
        if (binaryOpExp.Kind == S.BinaryOpKind.NotEqual)
        {
            ...
            {
                var castExp0 = context.TryCastExp_Exp(operandExp0, info.OperandType0);
                var castExp1 = context.TryCastExp_Exp(operandExp1, info.OperandType1);
        ...
        where TFuncDeclSymbol : IFuncDeclSymbol
    {
        ...
        var result = FuncMatcher.MatchIndex(context, bodyContext, localContext, TypeEnv.Empty, decls, sargs, typeArgsForMatch);
    ...
    ExpResult.Exp AnalyzeCallExpExpCallable(R.Loc callableLoc, IType callable, ImmutableArray<S.Argument> sargs, S.CallExp nodeForErrorReport)
    {
        ...
        var match = FuncMatcher.Match(context, bodyContext, localContext, outerTypeEnv, parameters, null, default, sargs);
    ...
    ExpResult.Exp AnalyzeCallExpEnumElemCallable(EnumElemSymbol enumElem, ImmutableArray<S.Argument> sargs, S.ISyntaxNode nodeForErrorReport)
    {
        ...
        var result = FuncMatcher.Match(context, bodyContext, localContext, TypeEnv.Empty, fieldParamTypes, null, default, sargs);
    ...
    ExpResult.Exp AnalyzeCallExpStructCallable(StructSymbol structCallable, ImmutableArray<S.Argument> sargs, S.CallExp nodeForErrorReport)
    {
        ...
        var result = FuncMatcher.MatchIndex(context, bodyContext, localContext,
    ...
    ExpResult AnalyzeIndexerExp(S.IndexerExp exp)
    {
        ...
        // 리스트 타입의 경우,
        if (context.IsListType(objResult.TypeSymbol, out var itemType))
        {
    ...
    // exp.x
    ExpResult AnalyzeMemberExpLocParent(S.MemberExp memberExp, R.Loc parentLoc, IType instanceType)
    {
    ...
    // T.x
    ExpResult AnalyzeMemberExpTypeParent(S.MemberExp nodeForErrorReport, IType parentType, string memberName, ImmutableArray<S.TypeExp> stypeArgs)
    {
    ...
    ExpResult AnalyzeMemberExp(S.MemberExp memberExp)
    {
        ...
        if (memberExp.Parent is S.IdentifierExp idParent)
        {
            ...
            var result = IdExpIdentifierResolver.Resolve(idParent.Value, typeArgs, ResolveHint.None, context, bodyContext, localContext);

            switch (result)
            {
                case IdentifierResult.NotFound:
                    context.AddFatalError(A2007_ResolveIdentifier_NotFound, idParent);
                    throw new UnreachableCodeException();
    ...
    ExpResult.Exp AnalyzeListExp(S.ListExp listExp)
    {
        ...
        IType? curElemType = (listExp.ElemType != null) ? context.GetSymbolByTypeExp(listExp.ElemType) : null;
    ...
    {
        var classSymbol = context.GetSymbolByTypeExp(newExp.Type) as ClassSymbol;
        if (classSymbol == null)
        ...
        var funcMatchResult = FuncMatcher.MatchIndex(context, bodyContext, localContext, classSymbol.GetTypeEnv(), constructorDecls, newExp.Args, default);
    ...
    {
        var result = context.TryCastExp_Exp(exp, expectedType);
    ...
    // 
    public static R.Exp AnalyzeExp_Exp(S.Exp exp, ResolveHint hint)
    {
        var analyzer = new StmtAndExpAnalyzer(context, bodyContext, localContext);
        return analyzer.AnalyzeExp_Exp(exp, hint);
    }

    // 결과
    public static R.Exp Visit(S.Exp exp, Context context, IType? hintType)
    {
