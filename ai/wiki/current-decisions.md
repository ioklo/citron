...
- `RAppliedDecl`의 type argument는 open type variable일 수 있으므로, 모든 argument가 적용됐다는 것과 closed declaration이라는 것은 구분한다. generic analysis에서는 적용 위치의 ordered lexical type-variable environment(`tenv`)를 항상 이용할 수 있어야 한다. 따라서 `List<int>`처럼 자체는 closed인 application도 `struct S<X> { struct T<Y> { ... } }` 안에서 만들어지면 tenv는 `[X, Y]`다. tenv는 shared immutable `RTypeEnv`로 표현하는 방향을 우선 검토하며, `RAppliedDecl` 직접 저장과 decl-space/body-space 제공 중 최종 위치는 미확정이다.
