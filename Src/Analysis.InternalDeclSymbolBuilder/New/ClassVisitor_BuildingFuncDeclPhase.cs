...
namespace Citron.Analysis
{
    class ClassVisitor_BuildingFuncDeclPhase
    {
        ...
        void VisitClassMemberFuncDecl(S.ClassMemberFuncDecl syntax)
        {
            ...
            var func = new ClassMemberFuncDeclSymbol(declSymbol, accessModifier, new M.Name.Normal(syntax.Name), typeParams, syntax.IsStatic);
        ...
        void VisitClassMemberVarDecl(S.ClassMemberVarDecl syntax)
        {
            foreach (var name in syntax.VarNames)
            {
                ...
                var declType = context.MakeTypeSymbol(declSymbol, syntax.VarType);

                // TODO: bStatic 지원
                var varDecl = new ClassMemberVarDeclSymbol(declSymbol, accessModifier, bStatic: false, declType, new M.Name.Normal(name));
            ...
            ClassConstructorSymbol? baseConstructor)
        {
            ...
            return new ClassConstructorDeclSymbol(outer, M.Accessor.Public, builder.MoveToImmutable(), bTrivial: true);
        }
        ...
        {
            ClassSymbol? uniqueBaseClass = null; // TODO: Object로 대체
            var interfacesBuilder = ImmutableArray.CreateBuilder<InterfaceSymbol>();

            foreach (var baseType in syntax.BaseTypes)
            {
                var baseTypeSymbol = context.MakeTypeSymbol(declSymbol, baseType);
                if (baseTypeSymbol is ClassSymbol baseClassSymbol)
                {
                    if (uniqueBaseClass != null)
                        throw new NotImplementedException(); // 베이스 클래스는 하나여야 합니다

                    uniqueBaseClass = baseClassSymbol;
                }
                else if (baseTypeSymbol is InterfaceSymbol interfaceSymbol)
                {
            ...
            context.RegisterTaskAfterBuildingConstructorDeclSymbols(uniqueBaseClass?.GetDecl(), declSymbol, context =>
            {
