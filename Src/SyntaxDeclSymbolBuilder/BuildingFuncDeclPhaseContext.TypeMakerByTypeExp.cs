...
partial class BuildingFuncDeclPhaseContext
{
    ...
    record struct TypeMakerByTypeExp(List<ModuleDeclSymbol> modules, SymbolFactory factory, IDeclSymbolNode node)
    {
        ...
        Candidates<Item> GetItems_IdTypeExp(S.IdTypeExp idTypeExp)
        {
            ...
            // outerPath를 한 단계씩 차례로 올려가면서 찾는다
        ...
        public IType MakeType(S.TypeExp typeExp)
        {
            ...
            {
                case UniqueQueryResult<Item>.Found foundResult:
                    if (foundResult.Value.Type != null)
                        return foundResult.Value.Type;
                    else
                        throw new NotImplementedException(); // 타입이 아닙니다.

                case UniqueQueryResult<Item>.MultipleError:
                    throw new NotImplementedException(); // 모호합니다

                case UniqueQueryResult<Item>.NotFound:
