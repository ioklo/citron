...
- `RAppliedDecl`의 type argument는 open type variable일 수 있으므로, 모든 argument가 적용됐다는 것과 closed declaration이라는 것은 구분한다. 적용 위치의 persistent `RTypeEnv`/`tenv`는 `RAppliedDecl`이나 `RType` identity에 보관하지 않는다. `RType_TypeVar`의 semantic binder identity는 positional index가 아니라 `RTypeParam*`다.
- generic member projection과 signature comparison에서 SmTranslator는 mirrored `SmType` hierarchy 대신 `SmTypeView { RType*, SmTypeSubstitution* }`를 사용한다. substitution은 `RTypeParam* formal -> SmTypeView actual` mapping의 lazy chain이며, 원본 canonical `RType`을 관찰할 때 필요한 부분만 resolve한다. actual도 caller substitution을 보존해야 하므로 bare `RType*`가 아니다.
- working copy에 merge된 `SmType` variant, index-based `SmType_TypeVar`, `SmTypeEnv { typeVarCount }`, `RTypeToSmType`은 이 결정 이전의 WIP prototype이며 현재 설계의 authoritative representation이 아니다. 구현 시 view/substitution 모델로 교체한다.
- declaration 자신에게 남은 type parameter가 없는 lambda, variable, enum element는 lexical outer arguments까지 적용된 뒤 `RAppliedDecl`로 나타낸다. 반대로 member 자신의 type arguments를 아직 받아야 하는 nominal type과 callable lookup은 `ROuterAppliedDecl` 또는 partial-application 상태를 유지한다. callable parameter projection은 own binder를 declaration context에 남긴 채 type view를 반환한다.
- trait requirement signature의 alpha-equivalence는 generic arity/constraint를 먼저 확인하고 requirement binder를 impl binder에 ordinal mapping한 뒤 exact contextual comparison으로 환원한다. fresh binder나 완전히 instantiate된 signature tree를 만들지 않는다.
- `RType::Apply`는 interning, serialization, lowering contract 등 canonical materialization이 필요한 경계의 helper로 남을 수 있으나, ordinary generic analysis의 기본 경로는 lazy `SmTypeView`다.
...
- 함수 overload lookup result는 `ROuterAppliedFuncDeclGroup<TFuncDecl>`으로 표현한다. source의 explicit function type arguments는 RSymbol lookup result에 결합하지 않고 SmTranslator의 partial-application state에 둔다. candidate matching은 explicit prefix와 outer contextual views를 사용해 남은 function binder의 substitution을 inference한다. canonical `RAppliedDecl`/full `RTypeArguments` materialization이 필요한 consumer boundary가 아니라면 이 과정에서 새 type tree를 만들지 않는다.
