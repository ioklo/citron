...
class ENamespaceDeclContainerComponent
{
    std::vector<ENamespaceDecl*> namespaceDecls; // preserve order
    std::unordered_map<std::string, ENamespaceDecl*> namespaceDict;

public:
    ENamespaceDeclContainerComponent();
    ETypeDecl* GetType(const EName& name);

    ESYMBOL_API void AddNamespace(ENamespaceDecl* _namespace);
    ESYMBOL_API ENamespaceDecl* GetNamespace(const std::string& name);
