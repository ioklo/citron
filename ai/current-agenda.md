...
- `some Trait`는 surface에서는 return-position marker지만 semantic에서는 `RType_Opaque`로 표현한다. opaque type identity는 Swift식으로 return declaration과 적용된 generic argument에 묶는다. 따라서 trait constraint만의 `$O<TraitTypeId>`로 intern하지 않으며, `RTypeIdentifier`/factory key에 opaque-result declaration identity를 포함한다.
