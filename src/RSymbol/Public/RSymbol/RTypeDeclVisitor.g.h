...
struct RTypeDeclVisitor
{
    ...
    virtual void Visit(RTypeAliasDecl* rTypeDecl) = 0;
};
...
concept RTypeDeclVisitable = requires(TVisitor&& v, TVisitorArgs&&... args)
{
    ...
    { v.Visit(std::declval<RTypeAliasDecl*>(), std::forward<TVisitorArgs>(args)...) } -> RTypeDeclConvertibleToResultType<TVisitor>;
...
typename std::remove_cvref_t<TVisitor>::ResultType Accept(TVisitor&& v, RTypeDecl* rTypeDecl, TVisitorArgs&&... args)
{
    ...
    if constexpr (std::is_void_v<TResult>)
    {
        struct Bridge : RTypeDeclVisitor {
            ...
            void Visit(RTypeAliasDecl* rTypeDecl) override { call(rTypeDecl); }
        };
    ...
    else
    {
        struct Bridge : RTypeDeclVisitor {
            ...
            void Visit(RTypeAliasDecl* rTypeDecl) override { result.emplace(call(rTypeDecl)); }
        };
