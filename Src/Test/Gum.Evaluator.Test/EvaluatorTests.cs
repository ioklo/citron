    ...
    public class EvaluatorTests
    {
        ...
        public async Task PrivateGlobalVariableExp_GetGlobalValueInFunc()
        {
            ...
            var topLevelStmts = new Stmt[]
            {
                ...
                new ExpStmt(new ExpInfo(new CallFuncExp(func0Id, null, Array.Empty<ExpInfo>()), TypeId.Void))
            };
        ...
        public async Task ReturnStmt_ExitsFuncImmediately()
        {
            ...
            var stmts = new Stmt[] { 
                new ExpStmt(new ExpInfo(new CallFuncExp(funcId, null, Array.Empty<ExpInfo>()), TypeId.Void)),
                PrintStringCmdStmt("Completed")
            };
        ...
        public async Task ReturnStmt_SetReturnValueToCaller()
        {
            ...
            var stmts = new Stmt[] {
                PrintIntCmdStmt(new CallFuncExp(funcId, null, Array.Empty<ExpInfo>())),
            };
        ...
        public async Task YieldStmt_GenerateElementForInnerMostForeachStmt()
        {
            ...
            var seqFunc0 = new SeqFunc(seqFunc0Id, TypeId.Int, false, Array.Empty<string>(), Array.Empty<string>(),
                new ForeachStmt(TypeId.Int, "elem", new ExpInfo(new CallSeqFuncExp(seqFunc1Id, null, Array.Empty<ExpInfo>()), TypeId.Enumerable),
                    new BlockStmt(
            ...
            var stmts = new[] {                
                new ForeachStmt(TypeId.Int, "x", new ExpInfo(new CallSeqFuncExp(seqFunc0Id, null, Array.Empty<ExpInfo>()), TypeId.Enumerable),
                    PrintIntCmdStmt(new LocalVarExp("x")))
            };
        ...
        public async Task CallFuncExp_EvaluatesArgumentsInOrder()
        {
            ...
            ExpInfo MakePrintCall(int v) =>
                new ExpInfo(new CallFuncExp(printFuncId, null, new[] { new ExpInfo(new IntLiteralExp(v), TypeId.Int) }), TypeId.Int);


            var stmts = new Stmt[]
            {
                new ExpStmt(new ExpInfo(
                    new CallFuncExp(testFuncId, null, new [] {
                        MakePrintCall(1),
                        MakePrintCall(2),
                        MakePrintCall(3) }
                    ),
        ...
        public async Task CallFuncExp_ReturnsValueProperly()
        {
            ...
            var stmts = new Stmt[] { PrintStringCmdStmt(new CallFuncExp(funcId, null, Array.Empty<ExpInfo>())) };
        ...
        public async Task CallSeqFuncExp_ReturnsEnumerableType()
        {
            ...
            {
                // 선언하자 마자 대입하기
                SimpleLocalVarDecl("x", TypeId.Enumerable, new CallSeqFuncExp(seqFuncId, null, Array.Empty<ExpInfo>())),

                // TODO: 지금은 value로 얻은 Enumerable을 조작할 수 있는 방법이 없다. foreach에서만 내부적으로 조작할 수 있는데, 그렇게만 하도록 할 것인지 결정을 해야 한다

                // 로컬 변수에 새 값 대입하기
                new ExpStmt(new ExpInfo(new AssignExp(new LocalVarExp("x"), new CallSeqFuncExp(seqFuncId, null, Array.Empty<ExpInfo>())), TypeId.Enumerable)),
            };
        ...
        public async Task CallValueExp_EvaluateCallableAndArgumentsInOrder()
        {
            ...
            var stmts = new Stmt[]
            {
                new ExpStmt(new ExpInfo(
                    new CallValueExp(
                        new ExpInfo(new CallFuncExp(makeLambdaId, null, Array.Empty<ExpInfo>()), TypeId.Lambda), 
                        new [] {
                            MakePrintCall(1),
                            MakePrintCall(2),
                            MakePrintCall(3) 
            ...
            var stmts = new Stmt[] {
                ...
                SimpleLocalVarDecl("func", TypeId.Lambda, new LambdaExp(
                    new CaptureInfo(false, new []{
                        new CaptureInfo.CopyLocalElement(TypeId.Int, "x"),
                    }),
        ...
        public async Task ListExp_EvaluatesElementExpInOrder()
        {
            ...
            Exp MakePrintCall(int v) =>
                new CallFuncExp(printFuncId, null, new[] { new ExpInfo(new IntLiteralExp(v), TypeId.Int) });
