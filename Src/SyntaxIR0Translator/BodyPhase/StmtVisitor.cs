...
namespace Citron.Analysis
{
    ...
    struct StmtVisitor
    {
        ...
        public StmtVisitor(ScopeContext scopeContext)
        {
        ...
        void VisitCommandStmt(S.CommandStmt cmdStmt)
        {
            ...
            var expVisitor = new ExpVisitor();
            foreach (var cmd in cmdStmt.Commands)
            {
                var expResult = expVisitor.VisitStringExp(cmd);
                Debug.Assert(expResult.Result is R.StringExp);
                builder.Add((R.StringExp)expResult.Result);
            }

            scopeContext.AddStmt(new R.CommandStmt(builder.ToImmutable()));
        }

        R.LocalVarDecl MakeLocalVarDecl(S.VarDecl varDecl)
        {
        ...
        void VisitLocalVarDeclStmt(S.VarDeclStmt varDeclStmt)
        {
            var localVarDecl = MakeLocalVarDecl(varDeclStmt.VarDecl);
