...
- declaration 자신에게 남은 type parameter가 없는 lambda, variable, enum element는 lexical outer arguments까지 적용된 뒤 `RAppliedDecl`로 나타낸다. 반대로 member 자신의 type arguments를 아직 받아야 하는 nominal type과 callable lookup은 `ROuterAppliedDecl` 또는 partial-application 상태를 유지한다.
