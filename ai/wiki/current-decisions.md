...
- `some Trait`의 semantic opaque type은 Swift식 declaration-scoped identity를 사용한다. identity는 return declaration과 적용된 generic argument로 unique하며, trait constraint가 같아도 서로 다른 declaration의 opaque result는 같은 static type이 아니다. `RType_Opaque`의 interning/canonical `RTypeIdentifier`에는 이 opaque-result declaration identity를 포함한다. 함수 body의 모든 return은 하나의 동일한 concrete underlying type으로 귀결되어야 한다.
