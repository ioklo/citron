...
예를 들어 requirement의 binder가 `[X.T1, Tr.T2, Tr.F.T3]`이고 impl 쪽의 대응
binder가 `[X.T1, impl.T5, impl.F.T6]`이면 requirement signature를 다음 lazy
substitution 아래에서 관찰한다.
...
원본 canonical `RType` tree를 새로 instantiate하지 않고, requirement의 반환형과
인자형을 `SmTypeView`로 projection할 때 필요한 substitution만 resolve한다. 그 결과
각각 `list<impl.T5>`, `impl.F.T6`이 되어 impl signature view와 exact contextual
type equality로 비교할 수 있다.

`GetGlobalIndex()`가 우연히 같다는 이유로 두 binder를 대응시키지 않는다. exact
identity는 `RTypeParam*`이며, alpha-equivalence는 이미 대응이 확인된 requirement
binder를 impl binder로 명시적으로 mapping한 뒤 exact comparison으로 환원한다.
함수 type parameter 개수와 제약, 인자 개수, 전달 방식도 함께 일치해야 한다.

compiler 표현과 substitution chain의 세부 규칙은
`ai/wiki/compiler/lazy-type-substitution.md`를 본다.
