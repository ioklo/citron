    ...
    partial struct StructVisitor_BuildingFuncDeclPhase
    {
        ...
        void VisitStructMemberVarDecl(S.StructMemberVarDecl syntax)
        {
            foreach (var name in syntax.VarNames)
            {
                ...
                // TODO: static지원
                var symbol = new StructMemberVarDeclSymbol(structDeclSymbol, accessModifier, bStatic: false, new M.Name.Normal(name));
                structDeclSymbol.AddMemberVar(symbol);

                var declTypeSymbol = context.MakeTypeSymbol(structDeclSymbol, syntax.VarType);
                symbol.SetDeclType(declTypeSymbol);
            }
        }
        ...
        public void VisitStructDecl(S.StructDecl syntax)
        {
            ...
            StructSymbol? uniqueBaseTypeSymbol = null; // 없다면 null일수도 있다. init으로는 넘겨줘야 한다
            var interfaceSymbolsBuilder = ImmutableArray.CreateBuilder<InterfaceSymbol>();
            foreach(var baseTypeSyntax in syntax.BaseTypes)
            {
                var baseTypeSymbol = context.MakeTypeSymbol(this.structDeclSymbol, baseTypeSyntax);
                
                if (baseTypeSymbol is StructSymbol structBaseTypeSymbol)
                {
                    if (uniqueBaseTypeSymbol == null)
                        throw new NotImplementedException(); // 두개 이상의 struct를 상속받았다면

                    uniqueBaseTypeSymbol = structBaseTypeSymbol;
                }
                else if (baseTypeSymbol is InterfaceSymbol interfaceBaseTypeSymbol)
                {
                    interfaceSymbolsBuilder.Add(interfaceBaseTypeSymbol);
                }
                else
                {
            ...
            this.structDeclSymbol.InitBaseTypes(uniqueBaseTypeSymbol, interfaceSymbolsBuilder.ToImmutable());

            // 다음은 constructor 빌드 단계
            var structDeclSymbol = this.structDeclSymbol;
            context.RegisterTaskAfterBuildingConstructorDeclSymbols(uniqueBaseTypeSymbol?.GetDecl(), structDeclSymbol, () =>
            {
                var baseTrivialConstructor = uniqueBaseTypeSymbol?.GetTrivialConstructor();                

                // baseStruct가 있고, TrivialConstructor가 없는 경우 => 안 만들고 진행
                // baseStruct가 있고, TrivialConstructor가 있는 경우 => 진행
                // baseStruct가 없는 경우 => 없이 만들고 진행 
                if (baseTrivialConstructor != null || uniqueBaseTypeSymbol == null)
                {
